...
[1] Structured Inference Networks for Nonlinear State Space Models [arXiv:1609.09869]
    ...
    Modification Copyright Sameer Khurana (2017)
"""
...
"""
TODO : Correctly Attribute to WNHsu's Code
"""

custom_relu = lambda x, cutoff: 0.5 * (x + torch.abs(x)) if cutoff == 0 else 0.5 * ((x - cutoff) + torch.abs(x - cutoff))
custom_double_relu = lambda x, scale: custom_relu(x, -1 * scale) - custom_relu(x, scale) + scale


class EncodeNuisance(nn.Module):
    """
    Parametrizes variational family q(z2 | x, z1)
    """
    def __init__(self, x_dim, hidden_dim, z2_dim,
                 layers=1, dropout=0.1, is_bidirectional=False):
        """

        """
        self.rnn = nn.GRU(input_size=x_dim, hidden_size=hidden_dim,
                          num_layers=layers, batch_first=True,
                          dropout=dropout, bidirectional=is_bidirectional)
        self.hidden_to_z1 = nn.Linear(hidden_dim, 2 * z2_dim)
        self.fc_mu = nn.Linear(2 * z2_dim, z2_dim)
        self.fc_sigma = nn.Linear(2 * z2_dim, z2_dim)
        self.layers = layers

    def forward(self, inputs, z1):
        """
        ...
        :return:
        """
        x = inputs['x'] # BS x T x F
        h0 = inputs['h0'] # BS x H
        z1 = z1.unsqueeze(1).repeat(1, x.size(1), 1)
        x = torch.cat([x, z1], dim=2)
        out, h = self.rnn(x, h0)
        if self.layers > 1:
            h = h.view(-1, h.size(-1) * h.size(-2))
        else:
            h = self.hidden_to_z1(h).squeeze()

        mu_z = self.fc_mu(h)
        sigma_z = torch.log1p(self.fc_sigma(h))
        return mu_z, sigma_z


class EncodeContent(nn.Module):
    """
    Parametrizes variational family q(z1 | x)
    """
    def __init__(self, x_dim, hidden_dim, z1_dim,
                 layers=1, dropout=0.1, is_bidirectional=False):
        """
        ...
        self.hidden_to_z1 = nn.Linear(hidden_dim, 2 * z1_dim)
        self.fc_mu = nn.Linear(2 * z1_dim, z1_dim)
        self.fc_sigma = nn.Linear(2 * z1_dim, z1_dim)
        self.layers = layers
    ...
    def forward(self, inputs):
        ...
        # TODO: ASk. Should we do a transformation or just split the 2 * z_dim vector from above
        mu_z = self.fc_mu(h)
        sigma_z = torch.log1p(self.fc_sigma(h))
        return mu_z, sigma_z
...
class Emitter(nn.Module):
    """
    Parameterizes the bernoulli observation likelihood `p(x | z_1, z_2)`
    """
    def __init__(self, input_dim, z_dim, emission_dim, layers):
        super(Emitter, self).__init__()
        # initialize the three linear transformations used in the neural network
        self.lin_z_to_hidden = nn.Linear(z_dim, emission_dim)
        self.lin_hidden_to_hidden = nn.Linear(emission_dim, emission_dim)
        self.lin_hidden_to_input = nn.Linear(emission_dim, 2 * input_dim)
        self.fc_mu = nn.Linear(2 * input_dim, input_dim)
        self.fc_sigma = nn.Linear(2 * input_dim, input_dim)
    ...
    def forward(self, z_t):
        ...
        """
        h = self.relu(self.lin_z_to_hidden(z_t))
        for i in range(self.layers):
            h = self.relu(self.lin_hidden_to_hidden(h))
        h = self.lin_hidden_to_input(h)
        mu_x = self.fc_mu(h)
        sigma_x = torch.log1p(self.fc_sigma(h))
        return mu_x, sigma_x
